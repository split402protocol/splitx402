...
| Router alpha | Implemented public-alpha router with static providers, control-plane route discovery, capability search, budget filtering, deterministic ranking, retry/fallback, pay-to wallet checks, and fail-closed receipt verification. |
...
commission/protocol-fee arithmetic from the receipt bps fields. Next is hosted
proof evidence from a real staging environment.
