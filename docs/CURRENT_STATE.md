...
| Router alpha | Implemented `@split402/router` public-alpha router with static providers, control-plane route discovery, external x402 onboarding discovery, capability search, budget filtering, deterministic ranking, retry/fallback, pay-to wallet checks, and fail-closed receipt verification. Plain external x402 routes are classified as onboarding candidates until they include Split402 campaign/offer metadata. |
| MCP demo bundle and gateway | Implemented public-alpha demo bundle and narrow stdio gateway: paid tool card, x402 payment metadata, Split402 campaign metadata, expected referral economics, router-backed `split402.searchCapabilities`, `split402.execute`, `split402.discoverExternalX402`, `split402.getReceipt`, optional control-plane route discovery, external x402 onboarding classification, and proof commands. It is a runnable demo gateway, not production MCP hosting. |
