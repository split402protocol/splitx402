...
Then start the same profile stack with
`corepack pnpm phase7:docker:compose up --brief --profile demo --profile workers`.
The command transcript must include `Split402 Phase 7 Docker compose up: ok`
and the command line must show both selected profiles.
Before seeding or collecting proof, run
`corepack pnpm phase7:docker:health --brief --profile demo --profile workers`.
The command transcript must include `Split402 Phase 7 Docker health: ready`
and `Profiles: demo, workers`.
...
- `commands_run` must be a local attached command transcript containing the
  ...
  doctor output must include `Profiles: demo, workers`, the pasted Docker
  compose output must include `Split402 Phase 7 Docker compose up: ok`, the
  pasted Docker health output must include `Split402 Phase 7 Docker health:
  ready`, and the pasted public-surface output must include `Split402 public
