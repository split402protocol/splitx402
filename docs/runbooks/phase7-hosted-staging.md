...
provider network/asset/amount/pay-to wallet/route id/referrer wallet/payout
wallet to match the receipt, and the reported paid amount to stay within that
budget. If hosted execution is enabled without a buyer signer, the collector
...
id, provider referrer wallet, receipt referrer wallet, provider payout wallet,
receipt payout wallet, commission bps, protocol-fee bps, commission amount, and
protocol-fee amount from the executed router call.
