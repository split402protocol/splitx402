# 0003: Adopt Split402 Architecture And `ffff` Baseline
...
The public `splitx402/ffff` repository was built from that architecture direction.
It contains the first implementation shape: protocol package, test vectors, x402
extension glue, Express adapter, agent SDK, demo merchant, demo agent, and public
demo runbooks.
...
they conflict with the architecture spec or `ffff`.

## Decision

Use the architecture spec as the canonical product and protocol scope. Use
`splitx402/ffff` as the implementation baseline for v2.

For this repository:

- preserve Split402 as the project/protocol name;
- keep `splitx402` only where it is a literal repo or GitHub path;
- port the `ffff` monorepo shape in stages;
- begin with deterministic protocol artifacts and test vectors;
- keep the Phase 1 service only as a temporary runnable host;
- make every intentional behavior drift from `ffff` explicit with fixtures, tests,
  and migration notes.
...
- The repo should avoid accidental behavior drift from `ffff`; drift should be
