...
function createNextCommands(input: {
  ...
  return [
    ...
    `corepack pnpm phase7:staging:apply-seed-env --seed-output ${input.directory}/phase7-seed.json`,
...
function createReadmeText(input: {
  ...
  return [
    ...
    "```",
    "",
    "On Windows PowerShell, replace leading inline environment assignments such",
    "as `SPLIT402_PHASE7_SEED_CONFIRM=seed-hosted-staging corepack ...` with",
    "`$env:SPLIT402_PHASE7_SEED_CONFIRM='seed-hosted-staging'; corepack ...`.",
    "The Phase 7 `commands_run` checker accepts PowerShell prompt lines in that",
    "form when validating the command transcript.",
    "",
    "Expected current status before evidence is collected:",
    "",
    "```text",
    "Launch gates ready: 0/3 (0%)",
    "Launch gates checked: 0/3 (0%)",
    "Mainnet ready: no",
    "```",
    "",
    ...
    "filled, that Docker readiness is checked on the hosted staging runtime,",
    "that hosted proof URLs are valid http(s) URLs, and that MCP live",
