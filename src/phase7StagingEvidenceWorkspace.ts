...
const PHASE7_STAGING_EVIDENCE_ARTIFACTS: readonly Phase7StagingEvidenceArtifact[] =
  [
    ...
    },
    {
      ...
      purpose:
        "MCP gateway stdio transcript showing router-backed discovery, quote preflight, execution, and receipt lookup.",
    },
    {
...
function createEnvText(directory: string): string {
  return [
    ...
    "# SPLIT402_PHASE7_WEBHOOK_STATUS=delivered",
...
function createReadmeText(input: {
  ...
  return [
    ...
    `SPLIT402_PHASE7_SEED_CONFIRM=seed-hosted-staging corepack pnpm phase7:staging:seed > ${input.directory}/phase7-seed.json`,
    `corepack pnpm phase7:staging:apply-seed-env --seed-output ${input.directory}/phase7-seed.json --phase7-env ${input.envFilePath}`,
    ...
    `corepack pnpm phase7:staging:commands-record --output ${input.directory}/commands.log`,
    ...
    "```",
    "",
    "Use commands-record to capture safe local validation output, then append",
    "the hosted Docker, seed, collection, paid-suite, manifest, assemble, and",
    "status command blocks from the real hosted staging run. Run commands-status",
    "before assembly to catch missing commands or placeholder output.",
    "The transcript must include the commands above plus lint, typecheck, test, build,",
    "vectors:check, and audit. Base and profile Docker doctor runs must report",
    "`Split402 Phase 7 Docker doctor: ready` from the hosted staging runtime,",
    "and the profile run must show `Profiles: demo, workers`. Docker health",
    "must report `Split402 Phase 7 Docker health: ready` for the same selected",
    "profile stack before proof collection continues.",
    "The MCP gateway collector",
    "report should include providerId, maxAmountAtomic, providerNetwork,",
    "providerAsset, providerMerchantOrigin, providerOperationId,",
    "providerCampaignId, providerAmountAtomic, providerPayToWallet,",
    "providerRouteId, providerReferrerWallet, providerPayoutWallet,",
    "executeProviderNetwork, executeProviderAsset,",
    "executeProviderMerchantOrigin, executeProviderOperationId,",
    "executeProviderCampaignId, executeProviderAmountAtomic,",
    "executeProviderPayToWallet, executeProviderRouteId,",
    "executeProviderReferrerWallet, executeProviderPayoutWallet, network,",
    "asset, merchantOrigin, operationId, campaignId, requiredAmountAtomic,",
    "payToWallet, amountPaidAtomic, receiptId, receiptVerificationStatus,",
    "receiptRecordingStatus, receiptRecordingSource, executeExecutionMode,",
    "referrerCreditAtomic, receiptReferrerCreditAtomic,",
    "receiptReferrerWallet, receiptPayoutWallet, routeId, commissionBps,",
    "protocolFeeBpsOfCommission, commissionAmountAtomic, and protocolFeeAtomic.",
    "The transcript should include the selected provider network, asset,",
    "merchant origin, operation id, campaign id, and payToWallet plus matching",
    "receipt network, asset, merchant origin, operation id, campaign id,",
    "payToWallet, routeId, referrer wallet, payout wallet, required amount, and",
    "referrer credit.",
    "The collector rejects execute evidence with missing required fields, missing",
    "or inconsistent execute-provider summary fields, an unsupported execution mode,",
    "an unverified receipt status, missing live receipt recording status,",
    "non-buyer receipt recording source, or zero referrer credit.",
    "`SPLIT402_MCP_MAX_AMOUNT_ATOMIC` must be a non-negative atomic amount",
    "string without decimals or leading zeroes; the MCP gateway collector stops",
    "before route discovery or execution when the budget is malformed.",
    ...
    "`SPLIT402_PHASE7_WEBHOOK_STATUS` is optional and, when set, must be one of",
    "`pending`, `processing`, `delivered`, or `dead_letter`.",
    "",
