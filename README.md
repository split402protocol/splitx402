...
| Capability router | Implemented public-alpha router with static providers, control-plane route discovery, budget filtering, deterministic ranking, fallback, pay-to wallet checks, fail-closed receipt verification, and optional verified-receipt recording to the control plane. |
...
| `@split402/router` | Public-alpha capability router with static providers, control-plane route discovery, external x402 onboarding discovery, GET/POST SVM/EVM provider execution, budget enforcement, deterministic ranking, retry/fallback, pay-to wallet checks, receipt verification, and optional verified-receipt recording for paid tools. |
...
| `@split402/mcp-demo` | MCP-facing paid-tool bundle and stdio gateway describing the demo tool, x402 payment requirement, Split402 campaign metadata, router-backed capability search/demo execution/external x402 onboarding/verified external router-ready provider loading/receipt lookup, route-attribution proof, receipt verification, live control-plane receipt recording when configured, and proof commands. |
...
corepack pnpm phase6:signer-policy:env-template split402-launch-evidence/phase6-signer-policy.env
